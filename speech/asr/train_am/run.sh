...
export PATH=/root/Desktop/wenet/wenet/bin:$PATH
...
data=/root/Desktop/speech/asr/train_am/data/huiyan_dataset  # fixme
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  # Data preparation
  python tools/data_split.py ${data}/raw_wav/uttid_wav_text.txt 0.05 ${data} false
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  for ((i = 0; i < $num_gpus; ++i)); do
  {
    ...
    python /root/Desktop/wenet/wenet/bin/train.py --gpu $gpu_id \
      --config $train_config \
      --data_type $data_type \
      --symbol_table $dict \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  if [ ${average_checkpoint} == true ]; then
    ...
    python /root/Desktop/wenet/wenet/bin/average_model.py \
  ...
  for mode in ${decode_modes}; do
  {
    ...
    mkdir -p $test_dir
    python /root/Desktop/wenet/wenet/bin/recognize.py --gpu -1 \
      --mode $mode \
