...
for room_name in "train_L" "train_M" "train_S" "test"; do
  ...
  find ${aishell4_source_dir}/$room_name/wav -iname "*.flac" >> ${aishell4_source_dir}/$room_name/wav_list.txt
  
  if [ -f ${aishell4_source_dir}/$room_name/TextGrid_list.txt ];then
    rm ${aishell4_source_dir}/$room_name/TextGrid_list.txt
  fi
  
  find ${aishell4_source_dir}/$room_name/TextGrid -iname "*.TextGrid" >> ${aishell4_source_dir}/$room_name/TextGrid_list.txt
  
done

mkdir -p ${aishell4_source_dir}/full_train
for r in train_L train_M train_S ; do
  cat ${aishell4_source_dir}/$r/TextGrid_list.txt >> ${aishell4_source_dir}/full_train/TextGrid_list.txt
  cat ${aishell4_source_dir}/$r/wav_list.txt >> ${aishell4_source_dir}/full_train/wav_list.txt
done
'''

wav_list_aishell4=${aishell4_source_dir}/full_train/wav_list.txt
text_grid_aishell4=${aishell4_source_dir}/full_train/TextGrid_list.txt
